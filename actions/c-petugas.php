...
if (ifset('tambah')) {
    ...
    if ($pw != $pw_konf) {
        ...
        if (rows($validasi) > 0) {
            ...
            $query = Qinsert("user", "nama_lengkap='$nama_lengkap', username='$username', email='$email', alamat='$alamat', password='$password', role='petugas' ");
...
if (ifset('edit')) {
    ...
    if ($pw != $pw_konf) {
        ...
        if (rows($validasi) > 0) {
            ...
            if (valid($pw)) {
                $query = Qupdate("user", "nama_lengkap='$nama_lengkap', username='$username', email='$email', alamat='$alamat', role='petugas' ", $id);
            } else {
                $query = Qupdate("user", "nama_lengkap='$nama_lengkap', username='$username', email='$email', alamat='$alamat', password='$password', role='petugas' ", $id);
            }
